...
export default function NumberWorldCBCPage() {
  ...
  useEffect(() => {
    ...
    setPlayer(current);
    const grade = getGrade(current.grade);
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto min-h-screen max-w-5xl px-5 py-6 sm:px-8">
        <header className="flex items-center justify-between gap-4">
          <Link href="/" className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-xl">←</Link>
        ...
        {!complete ? (
          <>
            ...
            <section className="mt-6 rounded-[2rem] border border-white/10 bg-white/5 p-7 sm:p-10">
              <div className="text-center"><div className="text-5xl">🧠</div><p className="mt-4 text-xs font-bold uppercase tracking-widest text-slate-500">Solve it</p><h2 className="mx-auto mt-3 max-w-3xl text-3xl font-black leading-tight sm:text-4xl">{currentQuestion.prompt}</h2></div>
              <div className="mx-auto mt-8 grid max-w-2xl gap-3 sm:grid-cols-2">
                {currentQuestion.options.map((option, index) => {
                  ...
                  return <button key={option.id} type="button" disabled={selected !== null} onClick={() => answer(option.id)} className={`rounded-2xl border p-5 text-left text-xl font-black transition ${isCorrect ? "border-emerald-400 bg-emerald-500/20 text-emerald-300" : isWrong ? "border-rose-400 bg-rose-500/20 text-rose-300" : "border-white/10 bg-white/5 hover:-translate-y-1 hover:bg-white/10"}`}><span className="mr-3 inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 text-sm">{String.fromCharCode(65 + index)}</span>{option.text}</button>;
                })}
              </div>
              {answerState && <div className={`mx-auto mt-6 max-w-2xl rounded-2xl p-5 text-center ${answerState === "correct" ? "bg-emerald-500/10" : "bg-orange-500/10"}`}><p className="text-lg font-black">{answerState === "correct" ? "🎉 Correct!" : "💡 Not quite."}</p><p className="mt-2 text-sm text-slate-300">{currentQuestion.explanation}</p>{answerState === "correct" && <p className="mt-2 font-black text-yellow-300">+{XP_PER_QUESTION} XP</p>}</div>}
            </section>
          </>
        ) : (
          <section className="mx-auto mt-12 max-w-3xl rounded-[2rem] border border-cyan-400/20 bg-gradient-to-br from-cyan-500/15 via-white/5 to-blue-500/10 p-8 sm:p-12">
            <div className="text-center"><div className="text-7xl">{mastery >= 80 ? "🏆" : mastery >= 50 ? "🌟" : "💪"}</div><p className="mt-6 text-sm font-bold uppercase tracking-widest text-cyan-300">Number World Complete</p><h2 className="mt-2 text-4xl font-black">{mastery >= 80 ? "Amazing work!" : "Keep practising!"}</h2></div>
            <div className="mt-8 grid grid-cols-3 gap-3"><div className="rounded-2xl bg-white/5 p-4 text-center"><p className="text-xs text-slate-500">SCORE</p><p className="mt-1 text-2xl font-black">{correctCount}/{questions.length}</p></div><div className="rounded-2xl bg-white/5 p-4 text-center"><p className="text-xs text-slate-500">MASTERY</p><p className="mt-1 text-2xl font-black text-cyan-300">{mastery}%</p></div><div className="rounded-2xl bg-white/5 p-4 text-center"><p className="text-xs text-slate-500">XP</p><p className="mt-1 text-2xl font-black text-yellow-300">+{earnedXP}</p></div></div>
            <div className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-5"><p className="text-xs font-bold uppercase tracking-widest text-slate-500">What KIDDO learned</p><div className="mt-4 space-y-3">{sessionResults.map((result) => <div key={result.id} className="flex items-center justify-between gap-4"><span className="text-sm font-bold text-slate-200">{result.concept ?? "Mathematics"}</span><span className={result.correct ? "font-black text-emerald-300" : "font-black text-orange-300"}>{result.correct ? "Secure signal" : "Needs practice"}</span></div>)}</div></div>
