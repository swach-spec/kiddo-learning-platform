...
export default function ChallengePage() {
  ...
  function next() {
    ...
    if (!getActivityResults(activePlayer.id).some((result) => result.activityId === session.completionActivityId && result.curriculumNodeId === (curriculumNodeId ?? undefined) && result.sessionId === sessionId && result.isSessionSummary === true)) {
  ...
  if (finished) {
    ...
    const progression = node ? getProgressionDecision(results, getEnglishPath(node.grade), node) : null;
    const destinationNode = progression?.outcome === "support" ? progression.node : progression?.nextNode;
    const destinationHref = destinationNode ? `${destinationNode.route}?node=${encodeURIComponent(destinationNode.id)}` : "/";
    const destinationLabel = progression?.outcome === "support" ? "Try a Support Activity →" : destinationNode ? `Continue to ${destinationNode.title} →` : "Continue Your Learning →";
    const headline = progression?.outcome === "support" ? "Let’s strengthen this skill." : progression?.outcome === "advance" ? "You’re ready for the next step!" : passed ? "Great work!" : "Good effort!";
    const message = progression?.outcome === "support" ? "KIDDO noticed that this idea needs more support before you move on." : progression?.outcome === "advance" ? "You have met the evidence needed for this step." : "Keep practising. KIDDO is using your work to decide what you need next.";

    return <main className="min-h-screen bg-slate-950 text-white"><div className="mx-auto max-w-2xl px-5 py-10"><section className="rounded-[2rem] border border-white/10 bg-white/5 p-8 text-center sm:p-12"><div className="text-6xl">{progression?.outcome === "support" ? "🧩" : passed ? "🎉" : "💪"}</div><h1 className="mt-4 text-4xl font-black">{headline}</h1><p className="mt-3 text-lg text-slate-400">You got <strong className="text-white">{score}</strong> of {session.challenges.length} correct.</p><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-slate-500">{message}</p><Link href={destinationHref} className="mt-8 block"><Button variant="primary">{destinationLabel}</Button></Link></section></div></main>;
  }
