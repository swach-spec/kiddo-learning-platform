...
export default function CheckersPage() {
  ...
  function finishGame(result: GameStatus, finalMoves = moves) {
    ...
    recordActivityResult({ id: `checkers-${player.id}-${Date.now()}`, playerId: player.id, activityType: "checkers", activityId: GAME_ID, skills: [], correct: true, attempts: finalMoves, hintsUsed: 0, xpAwarded: GAME_XP, timestamp: new Date().toISOString() });
    setRewarded(true);
  }
    ...
    <main className="kiddo-game-screen min-h-screen bg-slate-950 text-white">
      <div className="kiddo-game-shell mx-auto min-h-screen max-w-6xl px-5 py-6 sm:px-8">
        ...
        <section className="kiddo-game-stats mx-auto mt-6 flex max-w-3xl items-center justify-between rounded-3xl border border-white/10 bg-white/5 p-4">
          <div><p className="text-xs text-slate-500">YOUR PIECES</p><p className="text-2xl font-black">{redPieces}</p></div>
          <div className="text-center"><p className="text-xs text-slate-500">TURN</p><p className="text-lg font-black">{thinking ? "Computer thinking…" : turn === "red" ? "Your turn" : "Computer"}</p></div>
          <div className="text-right"><p className="text-xs text-slate-500">COMPUTER</p><p className="text-2xl font-black">{blackPieces}</p></div>
        </section>

        <section className="kiddo-game-stage mx-auto mt-8 w-full max-w-2xl overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-2 shadow-2xl sm:p-4">
          <div className="grid grid-cols-8 overflow-hidden rounded-2xl">{board.map((row, rowIndex) => row.map((piece, colIndex) => { const dark = (rowIndex + colIndex) % 2 === 1; const isSelected = selected?.row === rowIndex && selected?.col === colIndex; const isDestination = legalMoves.some((move) => move.to.row === rowIndex && move.to.col === colIndex); const key = `${rowIndex}-${colIndex}`; return <button key={key} onClick={() => handleSquareClick(rowIndex, colIndex)} className={`relative aspect-square flex items-center justify-center ${dark ? "bg-amber-900/70" : "bg-amber-100"} ${isSelected ? "ring-4 ring-inset ring-cyan-400" : ""}`} aria-label={`Row ${rowIndex + 1}, column ${colIndex + 1}`}>{isDestination && !piece && <span className="h-4 w-4 rounded-full bg-cyan-400/70 shadow-lg sm:h-5 sm:w-5" />}{piece && <span className={`flex h-[68%] w-[68%] items-center justify-center rounded-full border-4 shadow-lg transition ${piece.player === "red" ? "border-red-200/40 bg-red-600" : "border-slate-300/40 bg-slate-900"}`}>{piece.king && <span className="text-lg sm:text-2xl">👑</span>}</span>}</button>; }))}</div>
        </section>
