...
function chooseComputerMove(board: CheckersBoard): CheckersMove | null {
  ...
  return [...pool].sort((a, b) => {
    const aScore = (a.capture ? 10 : 0) + (a.to.row === 7 ? 3 : 0);
...
export default function CheckersPage() {
  ...
  useEffect(() => {
    ...
    setPlayer(current);
    setRewarded(
      getActivityResults(current.id).some(
        (result) =>
  ...
  function finishGame(result: GameStatus, finalMoves = moves) {
    ...
    recordActivityResult({
      id: `checkers-${player.id}-${Date.now()}`,
      playerId: player.id,
  ...
  function handleSquareClick(row: number, col: number) {
    ...
    if (piece?.player === "red") {
      const allMoves = getLegalMoves(board, "red");
      const fromThisPiece = allMoves.filter((move) =>
        samePosition(move.from, { row, col })
      );

      if (fromThisPiece.length > 0) {
        setSelected({ row, col });
        setLegalMoves(fromThisPiece);
        return;
      }
    }
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto min-h-screen max-w-6xl px-5 py-6 sm:px-8">
        <header className="flex items-center justify-between">
          <Link href="/games" className="text-sm font-bold text-slate-400 transition hover:text-white">
            ← Game Room
          </Link>
        ...
        <section className="mx-auto mt-6 flex max-w-3xl items-center justify-between rounded-3xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="text-center">
            ...
            <p className="text-lg font-black">{thinking ? "Computer thinking…" : turn === "red" ? "Your turn" : "Computer"}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-slate-500">COMPUTER</p>
            <p className="text-2xl font-black">{blackPieces}</p>
          </div>
        </section>

        <section className="mx-auto mt-8 w-full max-w-2xl overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-2 shadow-2xl sm:p-4">
          <div className="grid grid-cols-8 overflow-hidden rounded-2xl">
            {board.map((row, rowIndex) =>
              row.map((piece, colIndex) => {
                const dark = (rowIndex + colIndex) % 2 === 1;
                const isSelected = selected?.row === rowIndex && selected?.col === colIndex;
                const isDestination = legalMoves.some(
                ...
                return (
                  ...
                  >
                    {isDestination && !piece && (
                      <span className="h-4 w-4 rounded-full bg-cyan-400/70 shadow-lg sm:h-5 sm:w-5" />
                    )}
                    {piece && (
                      <span
                        className={`flex h-[68%] w-[68%] items-center justify-center rounded-full border-4 shadow-lg transition ${
                          piece.player === "red"
                            ? "border-red-200/40 bg-red-600"
                            : "border-slate-300/40 bg-slate-900"
                        }`}
                      >
                        {piece.king && <span className="text-lg sm:text-2xl">👑</span>}
                      </span>
                    )}
                  </button>
                );
